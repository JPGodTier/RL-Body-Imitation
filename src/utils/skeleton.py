...
def find_quaternions(topology, offsets, positions):
    ...
    quaternion[:, 0] = torch.Tensor([1.0, 0.0, 0.0, 0.0])
    ...
    for joint, parent in enumerate(topology):
        ...
        den = torch.linalg.norm(delta, dim=-1) * torch.linalg.norm(
            offsets[:, joint], dim=-1
        )
...
def find_rotations(topology, offsets, positions):
    ...
    quater[0] = np.array([1.0, 0.0, 0.0, 0.0])
    rel_quater = np.zeros_like(quater)
    rel_quater[0] = np.array([1.0, 0.0, 0.0, 0.0])
    ...
    for j in range(len(topology)):
        ...
        if np.linalg.norm(q_delta) > 0:
            ...
            q_delta = quat_left_multiply(
                quat_inverse(q_reference), quat_left_multiply(q_delta, q_reference)
            )
        ...
        rel_euler[j] = np.array([alpha, beta, 0.0])
...
def forward_kinematics(topology, base_position, offsets, quaternions, rel=False):
    ...
    for joint, parent in enumerate(topology):
        ...
        if rel:
            quaternions[:, joint] = batch_quat_left_multiply(
                quaternions[:, parent], quaternions[:, joint]
            )

        # Rotate the offset given the absolute rotation quaternion
        rel_positions[:, joint] = batch_rotate_vector(
            offsets[:, joint], quaternions[:, joint]
        )
...
def reorient_skeleton(skeleton, topology):
    ...
    for child, parent in enumerate(topology):
        lengths[:, child] = torch.sqrt(
            torch.sum((skeleton[:, child] - skeleton[:, parent]) ** 2, axis=-1)
        )
...
def extract_skeletons(skeletons_A, skeletons_B, alpha, depth_confidence=0.0):
    ...
    w = 0.5 * depth_confidence

    new_skeletons = torch.zeros_like(skeletons_A)

    new_skeletons[:, :, 0] = (1 - w) * skeletons_A[:, :, 0] + w * (
        np.cos(alpha) * skeletons_A[:, :, 1] - skeletons_B[:, :, 1]
    ) / np.sin(alpha)

    new_skeletons[:, :, 2] = 0.5 * (skeletons_A[:, :, 2] + skeletons_B[:, :, 2])

    new_skeletons[:, :, 1] = w * skeletons_A[:, :, 1] + (1 - w) * (
        skeletons_B[:, :, 0] - np.cos(alpha) * skeletons_A[:, :, 0]
    ) / np.sin(alpha)

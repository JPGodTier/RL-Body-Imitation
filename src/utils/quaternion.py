...
def rotate_vector(vector, quaternion):
    ...
    q_vector = np.zeros(4)
    q_vector[1:] = vector
    return quat_left_multiply(
        quaternion, quat_left_multiply(q_vector, quat_inverse(quaternion))
    )[1:]
...
def batch_rotate_vector(vector, quaternion):
    ...
    q_vector[:, 1:] = vector
    return batch_quat_left_multiply(
        quaternion, batch_quat_left_multiply(q_vector, batch_quat_inverse(quaternion))
    )[:, 1:]
...
def quaternion_to_euler_zxz(q):
    ...
    alpha = np.arctan2(
        2 * (q[0] * q[3] - q[1] * q[2]), q[0] ** 2 + q[2] ** 2 - q[1] ** 2 - q[3] ** 2
    )

    beta = np.arcsin((2 * (q[0] * q[1] + q[2] * q[3])).clip(-1, 1))

    gamma = np.arctan2(
        2 * (q[0] * q[2] - q[1] * q[3]), q[0] ** 2 + q[3] ** 2 - q[1] ** 2 - q[2] ** 2
    )
